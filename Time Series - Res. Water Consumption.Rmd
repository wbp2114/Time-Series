...
temp_celcius <- tsdl[[378]]


tsdl_london <- as.data.frame(cbind(water_consump, num_consumers, precipitation_ml, temp_celcius))
...
tsdl_london <-  tsdl_london %>% 
  mutate(
    season = case_when(
      ...
      month %in%  3:5  ~ "Spring",
      TRUE ~ "Summer"), 
    temp_fahrenheit = as.numeric(temp_celcius) * 9 / 5 + 32, 
...
dygraph(tsdl_london.ts[,c(10, 11, 3, 4)], 
        main = "London Water Consumption, Janaury 1983 - April 1994") %>% 
...
The dependent variable is __*water_consump*__, residential water consumption for the city of London between Janaury 1983 through April 1994. __*water_consump*__ is the consumption for the residential consumers who had their meter read in the given month for the last two months, and is considered a proxy for the total residential water conusmption. The independent variables are __*num_consumers*__ total residential consumers who had their water meters read in the given month for the last two months, __*precipitation_ml*__ monthly precipitation in millimeters, and __*temp_celcius*__ temperature in celcius. The number of residential consumers and residential water consumption were missing for June 1988 and an estimate (via intervention analysis) was provided in the dataset. __*Time in months*__ has a correlation coefficient of 0.9961 with year, and was used in lieu of year as the trend.
...
```{r, message = FALSE}
plotly::ggplotly(ggplot(tsdl_london, aes(x = num_consumers, y = water_consump)) + 
  geom_point() + 
  geom_smooth(method = "lm") + 
  theme_bw() +
  ggtitle("Residential Water Consumption, London, Janaury 1983 - April 1994") +
...
```{r, message = FALSE}
plotly::ggplotly(ggplot(tsdl_london, aes(x = precipitation_ml, y = water_consump)) + 
  geom_point() + 
  geom_smooth(method = "lm") + 
  theme_bw() +
  ggtitle("Residential Water Consumption, London, Janaury 1983 - April 1994") +
...
```{r, message = FALSE}
plotly::ggplotly(ggplot(tsdl_london, aes(x = temp_celcius, y = water_consump)) + 
  geom_point() + 
  geom_smooth(method = "lm") + 
  theme_bw() +
  ggtitle("Residential Water Consumption, London, Janaury 1983 - April 1994") +
...
```{r, message = FALSE}
plotly::ggplotly(ggplot(tsdl_london, aes(x = month, y = water_consump)) + 
  geom_point() + 
  geom_smooth(method = "lm") + 
  theme_bw() +
  ggtitle("Residential Water Consumption, London, Janaury 1983 - April 1994") +
...
```{r, message = FALSE}
plotly::ggplotly(ggplot(tsdl_london, aes(x = time_in_months, y = water_consump)) + 
  geom_point() + 
  geom_smooth(method = "lm") + 
  theme_bw() +
  ggtitle("Residential Water Consumption, London, Janaury 1983 - April 1994") +
...
```{r, message = FALSE}
plotly::ggplotly(ggplot(tsdl_london, aes(x = year, y = water_consump)) + 
  geom_point() + 
  geom_smooth(method = "lm") + 
  theme_bw() +
  ggtitle("Residential Water Consumption, London, Janaury 1983 - April 1994") +
...
lm.Dmodel_3 <- lm(firstD(water_consump) ~ firstD(num_consumers) + firstD(precipitation_ml) + firstD(temp_celcius) + firstD(month) + time_in_months, data = tsdl_london.ts)
...
```
The coefficients for __*ar1*__ (0.2298, s.e. = 0.237), __*ar2*__ (0.4696, s.e. = 0.1573), and __*ar3*__ (-0.4565, s.e. = 0.0769) are the weights for the first, second and third lags of the dependent variable, __*water_consump*__. The coefficient for __*ma1*__ (0.6382, s.e. = 0.2914) is the weighted sum of the current and lagged errors, for a lag of 1. After reducing the scale the y-variable __*water_consump*__ data by 10, the coefficients for the intercept, number of consumers and time in months variables were also reducted by a scale of 10. After returning to the normal scale, the coefficient for the __*intercept*__ is 2396518 (s.e. = 4554258), and it represents the water consumption, when all other independent variables are zero. After returning to the normal scale, the coefficient for __*num_consumers*__ is 1838.041 (s.e. = 129.852), and it represents the change in water consumption, all else equal, for one unit increase in number of consumers. After returning to the normal scale, the coefficient for __*time_in_months*__ is -40935.30 (s.e. = 33578.81), and it represents the change in water consumption, all else equal, for one unit increase in the monthly time trend.
